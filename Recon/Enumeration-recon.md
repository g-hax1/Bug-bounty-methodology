...
6. Crawl live web applications and merge all discovered URLs into the canonical endpoint file.
    Commands:
...
7. Run the JavaScript reconnaissance methodology.
...
8. Build a canonical parameter inventory without overwriting earlier results.
    Commands:
...
9. Scan non-web services only when service scanning is permitted by the programme.
    Commands:
...
10. Run only the automated checks allowed by scope, and keep raw results separate from verified findings.
    Commands:
...
11. Identify high-interest subdomains and prepare them for deep review.
...
2. Visit the domain and identify its technology, authentication flows, and main functionality.
    - Use the Wappalyzer extension and browser developer tools.
    - Record technologies in `technology-notes.md`, authentication observations in `authentication.md`, and important workflows in `business-logic.md`.

3. Crawl the host again with its known context and run the JavaScript methodology on newly discovered files.
    Commands:
...
5. Perform additional public-information research for the selected host.
...
6. Enumerate endpoint and API parameters only after baselining the response and confirming the method is safe.
    Commands:
